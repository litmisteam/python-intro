# Step 6: Watson - SpeechToText

In this section we will be using the Watson Developer Cloud SpeechToText API's to interface with Watson and translate an audio file containing human utterances \(aka someone talking\) into a text rendition of what was said. This has a vast array of use cases from low-level translation to high-level user-interaction integration.
...
3. Create the SpeechToText Watson service and return to the 'dashboard' page. From there, scroll down to your newly created SpeechToText service offering and select it. 
4. In the Speech to Text listing, go to the Service Credentials tab and take note of your username and password, these will be needed to access the service later. 
